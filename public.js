...
async function submitReview(courseId){const text=$("reviewText").value.trim();if(!text)return toast("Escribe tu opinión");if(!$("reviewConsent").checked)return toast("Necesitas autorizar la publicación");let photoUrl=null,f=$("reviewPhoto").files[0];if(f){const ext=(f.name.split(".").pop()||"jpg").replace(/[^a-z0-9]/gi,"");const path=`reviews/${Date.now()}-${crypto.randomUUID()}.${ext}`;const up=await db.storage.from(cfg.BUCKET).upload(path,f,{contentType:f.type,upsert:false});if(up.error)return toast("No se pudo subir la foto");photoUrl=db.storage.from(cfg.BUCKET).getPublicUrl(path).data.publicUrl}const ins=await db.from("reviews").insert({course_id:courseId,display_name:$("reviewName").value.trim()||"Anónimo",stars:+$("reviewStars").value,comment:text,photo_url:photoUrl,status:"pending",featured:false,consent:true});if(ins.error)return toast("No se pudo enviar: "+ins.error.message);toast("Opinión enviada para revisión");$("courseModal").classList.remove("show")}
function addCart(id){const c=courses.find(x=>x.id===id);if(!cart.find(x=>x.id===id))cart.push(c);renderCart();$("cartDrawer").classList.add("open")}function renderCart(){$("cartCount").textContent=cart.length;$("cartItems").innerHTML=cart.length?cart.map(c=>`<div class="cart-item"><div><strong>${c.name}</strong><br><small>${money(c.price)}</small></div><button class="btn danger remove-cart" data-id="${c.id}">Eliminar</button></div>`).join(""):`<p class="note" style="text-align:center">Tu carrito está vacío.</p>`;$("cartTotal").textContent=money(cart.reduce((s,c)=>s+Number(c.price),0));document.querySelectorAll(".remove-cart").forEach(b=>b.addEventListener("click",()=>{cart=cart.filter(x=>x.id!==+b.dataset.id);renderCart()}))}
function sendWA(){if(!cart.length)return toast("Agrega al menos un curso");const total=cart.reduce((s,c)=>s+Number(c.price),0),lines=cart.map(c=>`• ${c.name} — ${money(c.price)}`).join("\\n");window.open(`https://wa.me/${cfg.WHATSAPP}?text=${encodeURIComponent(`Hola Amonti 👋🏻\\n\\nMe gustaría inscribirme a estos cursos:\\n${lines}\\n\\nTotal: ${money(total)}\\n\\nQuedo pendiente de los datos para realizar mi transferencia o depósito. 😊`)}`,"_blank")}
$("cartBtn").addEventListener("click",()=>{renderCart();$("cartDrawer").classList.add("open")});$("closeCart").addEventListener("click",()=>$("cartDrawer").classList.remove("open"));$("sendWhatsApp").addEventListener("click",sendWA);$("closeModal").addEventListener("click",()=>$("courseModal").classList.remove("show"));$("courseModal").addEventListener("click",e=>{if(e.target===$("courseModal"))$("courseModal").classList.remove("show")});load();renderCart();